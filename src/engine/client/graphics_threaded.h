...
public:
	...
	// In-place update of a general RGBA texture. Without this, updating an
	// animated texture means destroying and re-creating it every frame, which
	// costs a full texture allocation plus a GPU-side delete of a texture that
	// may still be in use by the previous frame's draw calls.
	struct SCommand_Texture_Update : public SCommand
	{
		...
		size_t m_Width;
		size_t m_Height;
		// RGBA rows of m_Width*4 bytes; freed by the command processor
		uint8_t *m_pData;
	};
...
public:
	...
	bool UpdateTextureRgba(CTextureHandle TextureId, int x, int y, size_t Width, size_t Height, const uint8_t *pData) override;
