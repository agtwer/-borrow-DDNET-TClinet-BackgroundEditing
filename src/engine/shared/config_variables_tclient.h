...
// My custom: custom background (purely local visual feature)
MACRO_CONFIG_INT(McMenuBackground, mc_menu_background, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Show the custom background (png/mp4) on the main menu")
MACRO_CONFIG_INT(McGameBackground, mc_game_background, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Show the custom background (png/mp4) on the entity layer in game")
MACRO_CONFIG_STR(McBackgroundPath, mc_background_path, IO_MAX_PATH_LENGTH, "", CFGFLAG_CLIENT | CFGFLAG_SAVE, "Path to the custom background file (png/mp4, relative to the save folder or absolute)")
MACRO_CONFIG_INT(McBackgroundVideoFps, mc_background_video_fps, 30, 0, 120, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Max frame rate of the custom background video, 0 = source frame rate (visual only, lowers decode cost)")
MACRO_CONFIG_STR(McBackgroundFit, mc_background_fit, 32, "auto", CFGFLAG_CLIENT | CFGFLAG_SAVE, "Background fit resolution: auto, a screen size, or a fixed WxH like 1920x1440 (background crop/decode box only; never changes the game resolution)")
MACRO_CONFIG_INT(McBackgroundDiag, mc_background_diag, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Append background video diagnostics (decode rate, gate) to save/mcbg_diag.txt every 10s, 0 = off")
MACRO_CONFIG_INT(McBackgroundThreads, mc_background_threads, 4, 0, 64, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Decoder threads for the background video, 0 = auto (one per logical core); 4 measured fastest for 2K sources")
...
MACRO_CONFIG_INT(McMenuTitle, mc_menu_title, 1, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Show the big DDNet TClient title image on the main menu")
MACRO_CONFIG_INT(McBackgroundMode, mc_background_mode, 0, 0, 3, CFGFLAG_CLIENT | CFGFLAG_SAVE, "How the background fills the screen (never letterboxed): 0 = crop top/bottom (keep the whole width), 1 = crop left/right (keep the whole height), 2 = stretch, 3 = tile. Only one crop direction can avoid bars for a given video/screen, so the other falls back to it")
MACRO_CONFIG_INT(McBackgroundVideoRes, mc_background_video_res, 0, 0, 4, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Video decode resolution for the background: 0 = original source size (no downscale), 1 = 2160p, 2 = 1440p, 3 = 1080p, 4 = 720p. Lower it only if a large video is too heavy; it never depends on the screen")
