...
public:
	enum
	{
		...
		// Do not build a mipmap chain for this texture. Without it the driver
		// regenerates the mipmaps on every write to the texture, which makes
		// frequently updated (video) textures very expensive at large sizes.
	...
	// Repeat addressing: needed to tile a texture (draw the same image several
	// times). Restored with WrapNormal()/WrapClamp().
	virtual void WrapRepeat() = 0;
	...
	// In-place update of a general RGBA texture, so animated textures do not have
	// to be destroyed and re-created every frame. pData holds Height rows of
	// Width*4 RGBA bytes and is copied internally (the caller keeps ownership).
	// Width/Height must match the region being replaced; x/y select the offset.
	virtual bool UpdateTextureRgba(CTextureHandle TextureId, int x, int y, size_t Width, size_t Height, const uint8_t *pData) = 0;
