...
#include <game/client/components/wallpaper_engine.h>
...
private:
	...
	// My custom: local custom background (png/mp4), visual only
	CCustomMediaBackground m_CustomBackground;
	bool RenderCustomMenuBackground();
	void RenderSettingsMyCustom(CUIRect MainView);

	// Immersive view (Blue Archive style home button): hides every bit of menu
	// GUI except the toggle button itself, so only the background is left.
	// Runtime-only on purpose - it is never saved, so a fresh start always shows
	// the interface and the player can never end up stuck without it.
	bool m_HideGui = false;
	void RenderHideGuiButton(const CUIRect &Screen);

	// Background: discover and select a Wallpaper Engine wallpaper.
	CWallpaperEngine m_WallpaperEngine;
