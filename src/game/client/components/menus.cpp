...
int CMenus::DoButton_Menu(CButtonContainer *pButtonContainer, const char *pText, int Checked, const CUIRect *pRect, const unsigned Flags, const char *pImageName, int Corners, float Rounding, float FontFactor, ColorRGBA Color)
{
	...
	// Global opacity of the black panel behind rows/options (mc_ui_panel_alpha,
	// 0..25). 25 is the built-in default these panels are authored with, so the
	// factor is exactly 1.0 and nothing changes unless the user adjusts it.
	Color.a *= (float)std::clamp(g_Config.m_McUiPanelAlpha, 0, 25) / 25.0f;
...
void CMenus::OnInit()
{
	m_CustomBackground.Init(Graphics(), Storage());
...
void CMenus::RenderHideGuiButton(const CUIRect &Screen)
{
	// Small square in the free part of the start menu's bottom-right corner: the
	// version label (bottom) and the console button end 50px above the edge and
	// 50px further left, so this overlaps neither. Same Font Awesome icon family
	// as the rest of the menu (FontIcon::EYE / EYE_SLASH), so it blends into the
	// game's own style.
	constexpr float Size = 30.0f;
	constexpr float MarginRight = 12.0f;
	constexpr float MarginBottom = 50.0f;

	CUIRect Button;
	Button.x = Screen.x + Screen.w - Size - MarginRight;
	Button.y = Screen.y + Screen.h - Size - MarginBottom;
	Button.w = Size;
	Button.h = Size;

	static CButtonContainer s_HideGuiButton;
	// eye-slash = "hide the interface", eye = "bring it back"
	const char *pIcon = m_HideGui ? FontIcon::EYE : FontIcon::EYE_SLASH;
	// slightly stronger panel while hidden so the only remaining control stays
	// findable against a bright background
	const float PanelAlpha = m_HideGui ? 0.45f : 0.25f;
	if(Ui()->DoButton_FontIcon(&s_HideGuiButton, pIcon, 0, &Button, BUTTONFLAG_LEFT, IGraphics::CORNER_ALL, true, ColorRGBA(0.0f, 0.0f, 0.0f, PanelAlpha)))
	{
		m_HideGui = !m_HideGui;
	}
}
...
void CMenus::Render()
{
	...
	// "Hide all GUI" / immersive view (Blue Archive style home button). Available
	// on the start menu only: that is the "home page" showing the background, and
	// it is also the only menu screen whose bottom-right corner is free (on the
	// settings screen the category list owns that corner). Restricted to offline
	// with no popup, so a hidden interface can never hide a loading, connecting
	// or warning dialog.
	const bool HideGuiAvailable = ClientState == IClient::STATE_OFFLINE && m_Popup == POPUP_NONE && m_ShowStart;
	if(!HideGuiAvailable)
		m_HideGui = false;
	if(m_HideGui)
	{
		// Every other menu element is skipped, which also means no invisible
		// button stays clickable - only the toggle is registered with the UI.
		// Escape is consumed here as a second way out, so it restores the
		// interface instead of closing/deactivating the menu.
	...
	// Immersive view toggle, drawn on top of whichever menu page is showing.
	if(HideGuiAvailable)
		RenderHideGuiButton(Screen);
...
void CMenus::SetActive(bool Active)
{
	...
	m_MenuActive = Active;
	// never leave the interface hidden once the menu is closed
	if(!Active)
		m_HideGui = false;
	if(!m_MenuActive)
	{
...
void CMenus::UpdateColors()
{
	...
	// Global opacity of the black menu panels (the "shadows" the options sit on).
	// The tab buttons and the large box behind the settings content are all built
	// from the colours below, so scaling them here covers every panel of the menu
	// chrome at once; the section boxes and option rows drawn by this file's own
	// pages apply mc_ui_panel_alpha as well. 25 is the built-in default, so the
	// factor is exactly 1.0 and nothing changes unless the user adjusts it.
	// This runs every frame (see the caller), so the setting applies live.
	const float PanelAlpha = std::clamp(g_Config.m_McUiPanelAlpha, 0, 25) / 25.0f;

	ms_ColorTabbarInactiveOutgame = ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f * PanelAlpha);
	ms_ColorTabbarActiveOutgame = ColorRGBA(0.0f, 0.0f, 0.0f, 0.5f * PanelAlpha);
	ms_ColorTabbarHoverOutgame = ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f * PanelAlpha);
